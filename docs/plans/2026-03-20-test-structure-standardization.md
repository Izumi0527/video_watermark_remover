...
- [x] **Task 1 已完成**：`tests/unit/app/config/**` 迁移完成，定向验证为 `12 passed`。
- [x] **Task 2 已完成**：`video` 测试拆分到 `tests/unit/core/ai/video/` 与 `tests/integration/core/ai/video/`，定向验证为 `3 passed` 与 `4 passed`。
- [x] **Task 3 已完成**：AI / UI 历史测试已归位到 `tests/integration/ai/`、`tests/integration/ui/`，定向验证为 `8 passed, 5 skipped`。
- [x] **Task 4 已完成**：`legacy_phase3` 桥接目录已落地到 `tests/integration/legacy_phase3/`，桥接 smoke 验证为 `1 passed`。
- [x] **Task 5 已完成**：`tests/TESTING_GUIDE.md`、`scripts/vwr.ps1`、`docs/plans/2026-03-18-project-structure-audit.md` 已同步到新主分层结构。
- [x] **Task 6 已完成**：受影响集成层回归为 `13 passed, 5 skipped`，统一脚本门禁 `scripts/vwr.ps1 test unit -Quick` 为 `110 passed, 2 deselected`；当前机器直接裸跑 `pytest tests/unit -q` 会命中系统 `%TEMP%` 权限问题，因此以脚本提供的项目级运行时环境作为最终单测门禁。
...
### Task 1: 迁移配置类单测到 `unit/app/config`（已完成，2026-03-20）
...
### Task 2: 拆分 `video` 测试到 `unit` 与 `integration`（已完成，2026-03-20）
...
### Task 3: 归位顶层 AI / UI 历史测试到 `integration`（已完成，2026-03-20）
...
### Task 4: 迁移 `phase3` 历史 runner / support 到 `integration/legacy_phase3`（已完成，2026-03-20）
...
### Task 5: 清理空目录并同步脚本/文档（已完成，2026-03-20）
...
### Task 6: 分层回归与最终收口（已完成，2026-03-20）
