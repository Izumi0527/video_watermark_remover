...
def save_test_results(test_image: np.ndarray, processed_image: np.ndarray, 
                     mask: Optional[np.ndarray] = None, prefix: str = "test") -> bool:
    """
    保存测试结果图片
...
def validate_image_properties(image: np.ndarray, expected_shape: Optional[Tuple[int, ...]] = None) -> bool:
    """
    验证图像属性
