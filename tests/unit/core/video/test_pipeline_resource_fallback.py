...
def test_pipeline_resource_exhaustion_falls_back_to_singleprocess(
    ...
    frame_writer_module.frame_writer_worker = lambda *args, **kwargs: (
        False,
        "Frame writer fatal error: MemoryError()",
    )
    ...
    assert any("单进程" in item for item in processor.status.values)
