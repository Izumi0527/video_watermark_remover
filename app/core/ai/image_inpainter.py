...
class ImageInpainter:
    ...
    def inpaint_frame(self, frame: np.ndarray, mask: np.ndarray) -> np.ndarray:
        ...
        try:
            ...
            result_telea = cv2.inpaint(frame, mask, 3, cv2.INPAINT_TELEA)
            
            # 方法2: INPAINT_NS: Navier-Stokes方法(适合大区域)
            result_ns = cv2.inpaint(frame, mask, 3, cv2.INPAINT_NS)
            ...
            if total_inpaint_area < image_area * 0.05:  # 小区域(< 5%)
                result = result_custom
                method_used = "custom interpolation"
            elif total_inpaint_area < image_area * 0.15:  # 中等区域(< 15%)
                result = result_telea
                method_used = "TELEA"
            else:  # 大区域
                result = result_ns
