...
class WatermarkDetector:
    ...
    def detect_watermark(self, frame: np.ndarray) -> Optional[np.ndarray]:
        ...
        try:
            ...
            _, uniform_mask = cv2.threshold(diff, 10, 255, cv2.THRESH_BINARY_INV)
            
            # 方法3: 检测半透明覆盖层
            # 寻找特定饱和度特征的区域
            lower_semi = np.array([0, 30, 50])
            upper_semi = np.array([180, 100, 200])
            ...
            min_area = frame.shape[0] * frame.shape[1] * 0.001  # 至少占图像0.1%
